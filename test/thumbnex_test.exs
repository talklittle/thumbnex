defmodule ThumbnexTest do
  ...
  @fixture_invalid_video Path.join(__DIR__, "fixtures/cat_invalid_video.mp4")
  ...
  test "black image is smaller than cat image" do
    {:ok, nil} = Thumbnex.create_thumbnail(@fixture_video, @output_file, time_offset: 0)
    %{size: black_size} = File.stat!(@output_file)

    {:ok, nil} = Thumbnex.create_thumbnail(@fixture_video, @output_file, time_offset: 3)
  ...
  test "thumbnail of still image retains same dimensions" do
    ...
    {:ok, nil} = Thumbnex.create_thumbnail(@fixture_image, @output_file)
  ...
  test "preserve aspect when only one dimension specified, for still image" do
    {:ok, nil} = Thumbnex.create_thumbnail(@fixture_image, @output_file, width: 150)
    assert %{width: 150, height: 100} = @output_file |> Mogrify.open() |> Mogrify.verbose()

    {:ok, nil} = Thumbnex.create_thumbnail(@fixture_image, @output_file, height: 100)
    assert %{width: 150, height: 100} = @output_file |> Mogrify.open() |> Mogrify.verbose()
  end
  ...
  test "animated_gif_thumbnail/3 creates a GIF with 4 frames" do
    {:ok, nil} = Thumbnex.animated_gif_thumbnail(@fixture_video, @output_gif)
    assert %{frame_count: 4} = @output_gif |> Mogrify.open() |> Mogrify.verbose()
  end

  test "animated_gif_thumbnail/3 with frame_count: 10" do
    {:ok, nil} = Thumbnex.animated_gif_thumbnail(@fixture_video, @output_gif, frame_count: 10)
  ...
  test "animated_gif_thumbnail/3 with higher frame_count has higher duration, with fixed FPS" do
    {:ok, nil} = Thumbnex.animated_gif_thumbnail(@fixture_video, @output_gif, frame_count: 5)
  ...
  test "animated_gif_thumbnail/3 with higher FPS is shorter duration" do
    {:ok, nil} = Thumbnex.animated_gif_thumbnail(@fixture_video, @output_gif, fps: 1)
    duration_1 = Thumbnex.Gifs.duration(@output_gif)

    {:ok, nil} = Thumbnex.animated_gif_thumbnail(@fixture_video, @output_gif, fps: 2)
    duration_2 = Thumbnex.Gifs.duration(@output_gif)
  ...
  test "optimized GIF has smaller filesize than unoptimized" do
    {:ok, nil} = Thumbnex.animated_gif_thumbnail(@fixture_video, @output_gif, optimize: true)
    %{size: optimized} = File.stat!(@output_gif)

    {:ok, nil} = Thumbnex.animated_gif_thumbnail(@fixture_video, @output_gif, optimize: false)
  ...
  describe "Thumbnex.Animations.duration/1" do
    test "returns a duration for a gif" do
      assert {:ok, 1.0} == Thumbnex.Animations.duration(@fixture_gif)
    end
  end
